...
⚠️ **`SIBERIA_README.md` устарел**: в нём нет авторизации и не описаны дашборд, «Часы по фичам», «Проектная оценка», типы/классы задач.

## 🔥 Firebase и данные (из кода)

Конфиг в блоке `firebaseConfig` в `index.html`:

- **Firebase-проект: `siberia-777`** (`siberia-777.firebaseapp.com`) — миграция с `ihome-app777` выполнена, ветка `siberia-777-migration` слита в `main` и задеплоена. Ключи боевые: **менять их нельзя** — сайт переключится на другой проект и задачи пропадут.
- Снимок данных до миграции: тег `pre-siberia777` и локальная папка `backups/2026-09-11_pre-siberia777/` (в git не попадают).
...
- **Миграция на `siberia-777`**: выполнена, `main` задеплоен с боевыми ключами `siberia-777` — агенту писать в проект `siberia-777` (тот, откуда открыт сайт). Ветка `siberia-777-migration` и её копия на GitHub — историческая, рабочая ветка теперь `main`.
...
- **Как писать агенту**: элемент задачи лежит в Firestore, коллекция `siberia_tracker`, документ `tasks`, в `items` (карта `id → задача`). Меняйте `aiTracker`, **обязательно поднимая `updatedAt`** (`Date.now()`), иначе merge по `updatedAt` отбросит правку. Остальные поля задачи при записи нужно сохранять — элемент хранится целиком. Проект — `siberia-777` (тот же, что использует задеплоенный сайт).
...
- **Ключевые функции**:
  ...
  - **Калькулятор часов** (карточка `hourCalcCard` в левой панели, под Настройками): `renderHourCalc`, `calculateHours`, `saveHourCalcField`, `clearHourCalc`, `copyHourResult`. Состояние живёт **только в памяти** (`hourCalcState`, `loadHourCalcState`/`saveHourCalcState`) — при обновлении страницы поля пустые, последнее вычисление не сохраняется. Ключ localStorage `tracker_hourcalc` не используется и удаляется при загрузке (не возвращайте сохранение в localStorage — это была бага).
